...
Each teammate must create their own application.properties file using the example provided.

Never commit credentials (like MongoDB URIs, passwords, or tokens).

Use feature branches and make pull requests to main.

Ensure builds pass on GitHub before merging.

While creating PR make sure to select the appropriate reviewers and the fix or modification must be properly documented in the description.
